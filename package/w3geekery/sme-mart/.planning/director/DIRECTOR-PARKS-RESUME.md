...
3. Spec is fully LOCKED (Brian confirmed all 4 on 6-15; live form + CRM schema captured). Once admin access lands, work the **build checklist** in `contact-us-form-field-spec-2026-06-15.md`: create the 2 custom Lead fields (Business Classification picklist 7 values; Number of Employees bands `1-10/11-50/51-100/101-500/500+`) → build the Org-owned form (5 contact fields + 2 new + privacy notice + T&C) → wire form→Lead mapping, set a web-form Lead Source value, **preserve the `Review Status` approval gate (no auto-approve)** → swap onto `zerobias.com/contact-us/` (may need site edit access from Chris) → decommission the old `evaughn`-owned form.
